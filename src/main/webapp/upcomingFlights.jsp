...
<%
    ...
    try {
        ...
        // Get all upcoming flights for the current user (where travel_date is today or in the future)
        ...
        while (rs.next()) {
            ...
            // Check if this ticket can be canceled (only business or first class)
            String ticketType = rs.getString("ticket_type");
            boolean canCancel = "Business".equalsIgnoreCase(ticketType) || "First".equalsIgnoreCase(ticketType);
            flight.put("canCancel", canCancel);
            
            upcomingFlights.add(flight);
        }
    } catch (Exception e) {
        errorMessage = "Error retrieving upcoming flights: " + e.getMessage();
    } finally {
        if (rs != null) rs.close();
        ...
        <div class="flights-container">
            <% if (upcomingFlights.isEmpty()) { %>
                <div class="empty-state">
                    <h2>No Upcoming Flights Found</h2>
                    <p>You don't have any upcoming flight reservations.</p>
                    <a href="flightSearch.jsp" class="button">Search for Flights</a>
                </div>
            <% } else { %>
                <% for (Map<String, Object> flight : upcomingFlights) { %>
                    <div class="flight-card">
                        ...
                        <div class="flight-details">
                            ...
                            <div class="detail-column">
                                ...
                                <div class="detail-group">
                                    <% if ((Boolean)flight.get("canCancel")) { %>
                                        <a href="manageBookings.jsp?action=cancel&ticketId=<%= flight.get("ticketId") %>" class="cancel-button">Cancel Reservation</a>
                                    <% } else { %>
                                        <span class="cancel-disabled">Cancel Reservation</span>
                                        <p class="cancel-info">Only Business and First Class tickets can be canceled</p>
                                    <% } %>
                                </div>
                            </div>
                        </div>
